...
### 4.1 客户端配置审计基准（2026-08-05 新增）

对多客户端 MCP 配置发现与静态审计模块（`scanner/client_discovery.py`）做双基准，双向验证「不误报」与「不漏报」：

| 样本集 | 样本数 | 检出 | config_score | 说明 |
|--------|--------|------|--------------|------|
| 官方推荐配置（各客户端文档原样） | 20 | **0 误报** | **94/100** | 无任何 high/critical |
| 已知恶意/高危配置 | 10 | **10/10 全检出** | **0/100** | 提权、shell 注入、明文凭证、通配监听、远端无鉴权等 |

单 server 检查 10 类：提权启动、STDIO 执行暴露（`info`，零扣分）、启动时拉包、shell 解释器调用、非注册表来源、内联明文凭证、不安全传输、通配监听、远端无鉴权、项目级信任陷阱。
跨 server 检查 2 类：命名空间遮蔽、7 类毒性能力流。

**关键设计取舍**：全程**不执行**被扫配置中的任何命令。因此该模块**不覆盖运行时工具描述中的提示注入**（那部分由 201 条规则在直接提交 server 描述时覆盖）。取舍理由：MCP 配置的 `command` 是任意代码，若为读取 `tools/list` 而 spawn 进程，审计一份来自陌生仓库的项目级配置本身就构成一次执行。

**评分歧视度校准**：首版评分器对良性与恶意样本双双输出 0 分——恒定输出的评分器等价于无评分器。根因为低危项累计扣分无上限（server 数量压过风险严重度）。修复为 `{critical:20, high:8, medium:1.5, low:0.5, info:0}` 权重 + `{medium:20, low:5}` 累计上限，并新增 `TestScoringDiscrimination` 测试类锁定该性质。

> 复现：`python tests/run_all.py`（含 49 个配置审计测试，全套 190 测试）。
...
| 依赖卫生（安装脚本/锁文件） | ✅ | ⚠️ | ❌ | ⚠️ | ❌ | ✅ |
| 中文提示注入 | ✅ 31 条 | ⚠️ | ❌ | ❌ | ❌ | ❌ |
| 多客户端配置自动发现 | ✅ **14 面** | ❌ | ❌ | ✅ 8 面 | ⚠️ | ⚠️ |
| 审计时**不执行**被扫配置 | ✅ 纯静态 | N/A | N/A | ⚠️ 视模式 | ⚠️ | ⚠️ |
| 命名空间遮蔽 + 毒性流 | ✅ | ⚠️ | ❌ | ✅ 7 类 | ❌ | ⚠️ |

> **诚实标注**：测试规模上 AIShield（190）低于 mcp-audit（2,484）与 mcpaudit（6,430+）；深度 AST/taint 分析弱于 agent-security-scanner-mcp（1,000+ 规则、12 语言）。AIShield 的差异化不在"规则条数"，而在**双维覆盖 + 中性信任层 + 全离线幻觉包检测**这三项组合。
