...
## 周报 2026-09-07（自动巡检 · 2 项重大威胁：品牌劫持升级 + 直接竞品发新版）

> 本周最大变化：**aishield.ai 把其 LobeHub 条目的所有安装包链接直接指向我们的 GitHub 仓库 `lm203688/aishield`（品牌劫持从「蹭叙事」升级为「冒用仓库借信用」）**；同时**直接竞品 `agent-security-scanner-mcp` 发布了 3.1.0 重大版本**（1700+ 规则 / 自动修复 / 幻觉包检测 / SARIF），能力大幅扩张。我们自身 4.3.0 线上健康（commit 08a75eb，2026-09-06 部署）。

- 🔴 **[重大·品牌劫持升级]** LobeHub `aishield-ai/aishield` 现 **v2.1.0 / Updated 2026-08-31 / 标 "Unvalidated"**，其「Packages」区把 `aishield-mcp`、`aishield-guardrail`、`aishield`(PyPI)、GitHub Action `aishield/audit@v1`、Claude Skill **全部链接到 `github.com/lm203688/aishield`（我们的仓库）**，并仍标「Local Service / 本地」。但实测这些包均不存在：`aishield-mcp`(npm 404)、`aishield-guardrail`(npm 404)、`github.com/aishield/audit`(GitHub 404)。→ 这是**从「蹭本地叙事」升级为「冒用我们的开源仓库借信用」**的明确品牌劫持，制造「aishield.ai = 我们的开源项目」混淆。净判断：域名仍 Atom 待售（$19,495）但 LobeHub 列表主动维护 → parked 域名 + 冒用我们 repo 两头套利。需用户在 LobeHub 提交侵权/混淆申诉（AI 不代发）。
- 🔴 **[重大·直接竞品发新版]** `agent-security-scanner-mcp`(sinewaveai) 已发 **3.1.0（约 2026-09-06，mcp.so / mdskills.ai 已上架）**，相比上周在册的 2.0.1 能力大扩张：1700+ 规则 / 12 语言 tree-sitter AST + taint 分析、幻觉包检测（4.3M+ 包跨 7 生态）、prompt injection 56 规则、**自动修复（fix_security）**、**SARIF 2.1.0 对接 GitHub Advanced Security / GitLab SAST**。`mcp.so` 上还出现 `agent-security-scanner-mcp/Prooflayer` 分叉 → 碎片化。⚠️ **Official MCP Registry 仍显示 2.0.1 isLatest（3.1.0 尚未同步，Registry 滞后）**。该竞品补上了「自动修复 + 企业 SAST 闭环」，直接对冲我们的「内容安全平面 + 主动治理」楔子，差异化须上移至 **机器可结算认证（x402）+ 持续鉴证 attestation** 这类它没做的层。
- 🟠 **[常规·新晋小竞品]** dev.to 综述曝新玩家：**Shrike MCP**(Shrike-Security/shrike-mcp，agent 循环运行时扫描 prompt/response/SQL/file/shell/web)、**MCP Injection Guard**(Hosein-Abdollahi，provenance/taint 追踪)、**MCP-Guard**(GenTelLab，对抗性 MCP prompt 基准)。均小而专，作监控项。
- 🟢 **[我们·线上健康]** aishield.tools `GET /api/v1/health` → **4.3.0 / rules_count 228 / commit 08a75eb / deployed 2026-09-06T08:48**；`.well-known/mcp/server-card.json` 4.3.0 / 227+233 / 工具名正确；Official MCP Registry `io.github.lm203688/aishield` **4.3.0 isLatest=true active**（09-07 复测同态）；npm `aishield-mcp-server` latest **4.3.0**（modified 2026-08-28）。
- ⚠️ **[持续·npm README drift]** 4.3.0 的 npm README 仍写「201 local rules / Total 215(MCP)·221(Skill)」，与真相 227/233 不符（低估且内部自相矛盾）→ 下次发版同步（AI 不代改）。
- ℹ️ **[ClawHub squat 稳定]** `ai-shield-audit`(laurentaia) 仍在，跨 clawskills.sh / claudemarket.ai(评分 Grade D 38/100，1 安装) / clawbox.com(4.7★，€549–€579) / openclawskills.wiki；v1.0.0 / Updated 5/17/2026。无全新 aishield 同名 squat，既有劫持面未扩大。

**行动建议（用户侧，AI 不代发/不代提）**：① **在 LobeHub 对 `aishield-ai/aishield` 提品牌混淆/冒用申诉**（链接指向我们 repo）；② 评估回收 `aishield.ai` 域名（仍 Atom 待售 $19,495，且冒用升级 → 杠杆上升）；③ 防御性抢注 npm `aishield-mcp`（仍 404 空闲）阻断劫持；④ 竞品 3.1.0 已补自动修复+SAST 闭环，差异化坚定上移「机器可结算认证(x402)+持续鉴证」；⑤ 下次发版同步 npm README 到 227/233。
...
| **npm README 规则数 drift** | ⚠️ 公开面低估（4.3.0 仍存） | — | 发版同步 | LOW | **2026-08-31 复测**：npm 已发布 README 写「**201 local rules / Total 215(MCP)·221(Skill)**」，与 `get_rule_count()` 真相 **227 MCP / 233 Skill**（基线）仍不符（低估 ~12/12，且内部 110+60+23+8=201 与 Total 215 自相矛盾）。4.3.0 未修正 → 建议下次发版同步 npm README 数字（AI 不代改）；**2026-09-07 复测 npm 已发布 README 仍写「201 local rules / Total 215(MCP)·221(Skill)」，与 `get_rule_count()` 真相 227 MCP / 233 Skill 不符（低估且内部 110+60+23+8=201 与 Total 215 自相矛盾），持续 drift** |
...
#### 2026-09-07 竞品更新（直接竞品发 3.1.0 + 新晋小玩家，差异化须上移）
