...
1. **awesome-mcp-servers / Cursor / MCP Registry 上架复核**（文案见 `docs/listing-copy.md`）。
2. **向 ANS / ai-catalog 工作组提 `trust` 字段**（复用同一提案）。
3. **Authentik 集成**：把 `identity_scan` 做成可扫 Authentik service-account 导出的模块，发到 Authentik 社区。

### P2 — 内容/文化层（差异化叙事）

1. **soundshuman / de-AI 社区桥接**：明确分工——他们是风格层，我们是安全/可信层；可做一篇"agent 内容安全 vs 去 AI 味"的定位文，避免被误认为竞品。
2. **内容溯源 / AI-slop 规避检测**（路线图模块）：识别专门绕过检测器的恶意 prompt/skill。
...
### 下一步（路线图）—— ✅ 已于 2026-08-12 全部实做并接入 `_local_pipeline`

- ✅ **内容溯源 / AI-slop 规避检测**（`scanner/slop_scan.py`）：识别为绕过检测器而设计的 prompt/skill（指令覆盖 / DAN 角色扮演 / base64·翻译混淆 / 反检测自指）。
- ✅ **支付授权 scope 审计**（`scanner/payment_scan.py`）：扫描 AP2/x402 支付授权过宽（maxAmount 无限 / auto_approve / scope 过宽 / 未绑 intent）。
- ✅ **Authentik 导出扫描器**（`scanner/authentik_scan.py`）：扫 service-account 导出（skip_authorization / token 不过期 / scope 过宽 / 硬编码密钥）。
- ✅ **A2A AgentCard 校验器**（`scanner/agentcard_scan.py`）：结构化校验 `.well-known/agent.json`（未签名 / 无过期 / 非 https / 无鉴权方案 / 委托未收敛）。

> 四模块均经 `tests/test_capability_boundary_scan.py`（26 项）覆盖，全量测试 **483 通过 / 0 失败 / 9 skipped**，良性零误报、恶意仍被拦。与既有 identity_scan / network_scan 共用 `extra_findings` 并入 5 维评分。
